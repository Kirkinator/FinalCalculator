...
int main()
{
	...
	FILE* fptr;
	if ((fptr = fopen("C:\\Users\\Chris\\Desktop\\userfile.txt", "r")) == NULL) {
		printf("Error!");
	...
	union Data data; //union to display copyright information
	data.date = 2021;
	printf("\npublication date : %d\n", data.date);
	...
	printf("Developed by : %s\n", data.string); //displays developer, version, and publication date
	...
	scanf("%s\n", &s1.job);
	display(s1);// this will display the user's name and job you listed at the beginning
